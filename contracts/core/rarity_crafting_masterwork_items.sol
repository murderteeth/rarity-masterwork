...
interface masterwork_projects {
  function projects(uint token) external view returns (masterwork_uri.Project memory);
}
...
contract rarity_masterwork_items is ERC721 {
  ...
  function claim(
    ...
    require(Crafting.isApprovedOrOwnerOfItem(projectToken, PROJECT_MINT), "!approvedForItem");
    require(!claims[projectToken], "claimed");

    masterwork_uri.Project memory project = masterwork_projects(PROJECT_MINT).projects(projectToken);
