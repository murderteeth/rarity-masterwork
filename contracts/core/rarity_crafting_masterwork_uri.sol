...
library MasterworkUri {
  ICodexSkills constant SKILLS_CODEX = ICodexSkills(0x67ae39a2Ee91D7258a86CD901B17527e19E493B3);
  ...
  function item_name(uint8 base_type, uint8 item_type) public pure returns (string memory result) {
    if(base_type == 2) {
  ...
  function project_uri(uint token, Project memory project, Item memory item, uint m, uint n) public pure returns (string memory) {
    uint y = 0;
    ...
    y += 20; svg = string(abi.encodePacked(svg, '<text x="10" y="', StringUtil.toString(y), '" class="base">', 'ended ', ended_string(project, item), '</text>'));
  ...
  function armor_uri(uint token, Item memory item) public pure returns (string memory) {
    uint y = 0;
  ...
  function weapon_uri(uint token, Item memory item) public pure returns (string memory) {
    uint y = 0;
  ...
  function tools_uri(uint token, Item memory item) public pure returns (string memory) {
    uint y = 0;
  ...
  function base_type_name(uint8 base_type) internal pure returns (string memory result) {
    if (base_type == 2) {
      result = "Armor";
    } else if (base_type == 3) {
      result = "Weapons";
  ...
  function progress_string(uint m, uint n) internal pure returns (string memory) {
    return string(abi.encodePacked(StringUtil.toString(m * 100 / n), "%"));
  }

  function ended_string(Project memory project, Item memory item) internal pure returns (string memory) {
    if(project.complete) {
  ...
  function tools_bonus_svg_fragment(ITools.Tools memory tools, uint y) internal pure returns (string memory result, uint new_y) {
