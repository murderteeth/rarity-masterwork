...
library AdventureUri {
  ...
  function tokenURI(
    ...
    y += 20; svg = string(abi.encodePacked(svg, '<text x="10" y="', StringUtil.toString(y), '" class="base">', 'status ', status_string(adventure, turn_order), '</text>'));
    y += 20; svg = string(abi.encodePacked(svg, '<text x="10" y="', StringUtil.toString(y), '" class="base">', 'summoner ', summoner_string(adventure, turn_order), '</text>'));

    y += 20; (string memory loadout_fragment, uint y_after_loadout) = loadout_svg_fragment(loadout, y);
    svg = string(abi.encodePacked(svg, loadout_fragment));

    y = y_after_loadout + 20; (string memory monster_fragment, uint y_after_monsters) = monsters_svg_fragment(turn_order, monsters, y);
    svg = string(abi.encodePacked(svg, monster_fragment));

    y = y_after_monsters; svg = string(abi.encodePacked(svg, '<text x="10" y="', StringUtil.toString(y), '" class="base">', 'loot ', loot_string(adventure, turn_order, monsters), '</text>'));
  ...
  function status_string(Adventure memory adventure, Combat.Combatant[] memory turn_order) public pure returns (string memory result) {
  ...
  function summoner_string(Adventure memory adventure, Combat.Combatant[] memory turn_order) public pure returns (string memory result) {
    ...
    for(uint i = 0; i < turn_count; i++) {
      if(turn_order[i].mint == RARITY) {
        result = string(abi.encodePacked(
          result, 
          " (", StringUtil.toString(turn_order[i].hit_points), "hp)"
        ));
      }
    }
  }

  function monsters_svg_fragment(Combat.Combatant[] memory turn_order, uint8[] memory monsters, uint y) public pure returns (string memory result, uint new_y) {
      ...
      for(uint i = 0; i < turn_count; i++) {
        if(turn_order[i].mint != RARITY) {
          result = string(abi.encodePacked(
            ...
            Monster.monster_by_id(monsters[monster_index++]).name,
  ...
  function loadout_svg_fragment(Equipment.Slot[3] memory loadout, uint y) public view returns (string memory result, uint new_y) {
  ...
  function outside_dungeon(AdventureUri.Adventure memory adventure) public pure returns (bool) {
    return !adventure.dungeon_entered 
    && adventure.ended == 0;
  }

  function en_combat(AdventureUri.Adventure memory adventure) public pure returns (bool) {
    return adventure.dungeon_entered 
    && !adventure.combat_ended
    && adventure.ended == 0;
  }

  function combat_over(AdventureUri.Adventure memory adventure) public pure returns (bool) {
    return adventure.dungeon_entered 
    && adventure.combat_ended
    && adventure.ended == 0;
  }

  function ended(AdventureUri.Adventure memory adventure) public pure returns (bool) {
    return adventure.ended > 0;
  }

  function victory(AdventureUri.Adventure memory adventure) public pure returns (bool) {
    return adventure.monster_count == adventure.monsters_defeated;
  }

  function fled(AdventureUri.Adventure memory adventure, Combat.Combatant[] memory turn_order) public pure returns (bool result) {
    if(combat_over(adventure) && adventure.monster_count > adventure.monsters_defeated) {
      uint turn_count = turn_order.length;
      for(uint i = 0; i < turn_count; i++) {
        if(turn_order[i].mint == RARITY) {
  ...
  function count_loot(AdventureUri.Adventure memory adventure, Combat.Combatant[] memory turn_order, uint8[] memory monsters) public view returns (uint) {
    ...
    for(uint i = 0; i < turn_count; i++) {
      ...
      if(combatant.mint == address(this)) {
        reward += Monster.monster_by_id(monsters[monster_index++]).challenge_rating;
      }
    }
  ...
  function loot_string(Adventure memory adventure, Combat.Combatant[] memory turn_order, uint8[] memory monsters) public view returns (string memory result) {
    result = "--";
    if(ended(adventure) && victory(adventure)) result = string(abi.encodePacked(StringUtil.toString(count_loot(adventure, turn_order, monsters) / 1e18), " Salvage"));
  }
}