...
contract RaritySkillCheck {
    ...
    function survival(uint256 summonerId) public pure returns (int8) {
        int8 survival = int8(SKILLS.get_skills(summonerId)[31]);
        (, , , , uint256 wisdom, ) = ATTRIBUTES.ability_scores(summonerId);
        return skillCheck(survival, wisdom);
    }

    function senseMotive(uint256 summonerId) public pure returns (int8) {
