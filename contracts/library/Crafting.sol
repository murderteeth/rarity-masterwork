...
library Crafting {
  function isApprovedOrOwnerOfItem(uint token, address mint) public view returns (bool) {
    if(IERC721(mint).getApproved(token) == msg.sender) return true;
    address owner = IERC721(mint).ownerOf(token);
    return owner == msg.sender || IERC721(mint).isApprovedForAll(owner, msg.sender);
  }
}