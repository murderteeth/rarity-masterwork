...
export async function mockMasterwork (
  library: IMockLibrary,
  rarity: IMockRarityContracts
): Promise<IMockMasterworkContracts> {
  const weaponsCodex = await (
    await smock.mock<CodexMasterworkWeapons__factory>('CodexMasterworkWeapons')
  ).deploy()
  const projects = await (
    await smock.mock<RarityMasterworkProject__factory>(
      'RarityMasterworkProject'
    )
  ).deploy()
  ...
  await rarity.core.setVariable('_owners', {
    [(await projects.APPRENTICE()).toNumber()]: projects.address
  })
  const toolsCodex = await (
    await smock.mock<CodexMasterworkTools__factory>('CodexMasterworkTools')
  ).deploy()
  const items = await (
    await smock.mock<RarityMasterworkItem__factory>('RarityMasterworkItem')
  ).deploy()
  ...
  const commonToolsCodex = await (
    await smock.mock<CodexCommonTools__factory>('CodexCommonTools')
  ).deploy()
  const commonTools = await (
    await smock.mock<RarityCommonTools__factory>('RarityCommonTools')
  ).deploy()
  ...
  const barn = await (
    await smock.mock<RarityKoboldBarn__factory>('RarityKoboldBarn', {
      libraries: {
...
export async function mockRarity (): Promise<IMockRarityContracts> {
  ...
  const random = await (
    await smock.mock<CodexBaseRandomMockable__factory>(
      'codex_base_random_mockable'
    )
  ).deploy()
  const attributes = await (
    await smock.mock<RarityAttributes__factory>('rarity_attributes')
  ).deploy()
  await attributes.setVariable('rm', core.address)
  const gold = await (
    await smock.mock<RarityGold__factory>('rarity_gold')
  ).deploy()
  await gold.setVariable('rm', core.address)
  const skills = await (
    await smock.mock<RaritySkills__factory>('rarity_skills')
  ).deploy()
  await skills.setVariable('rm', core.address)
  await skills.setVariable('_attr', attributes.address)
  const mats = await (
    await smock.mock<RarityCraftingMaterials__factory>(
      'rarity_crafting_materials'
    )
  ).deploy()
  await mats.setVariable('rm', core.address)
  await mats.setVariable('_attr', attributes.address)
  const crafting = await (
    await smock.mock<RarityCrafting__factory>('rarity_crafting')
  ).deploy()
...
export async function mockCrafter (
  rarity: IMockRarityContracts,
  ...
  await rarity.core.setVariable('_owners', {
    [summoner.toNumber()]: signer.address
  })
  await rarity.core.setVariable('_owners', {
    [(await rarity.crafting.SUMMMONER_ID()).toNumber()]: rarity.crafting.address
  })
  await rarity.core.setVariable('class', { [summoner.toNumber()]: 11 })
  await rarity.core.setVariable('level', { [summoner.toNumber()]: 5 })
  await rarity.core.setVariable('xp', {
    [summoner.toNumber()]: parseEther('10000')
  })
  await rarity.core.setVariable('next_summoner', summoner.add(1))
  await rarity.gold.setVariable('balanceOf', {
    [summoner.toNumber()]: parseEther('10000')
  })
  await rarity.attributes.setVariable('ability_scores', {
    [summoner.toNumber()]: {
      strength: 8,
      dexterity: 8,
      constitution: 10,
  ...
  await rarity.attributes.setVariable('level_points_spent', {
    [summoner.toNumber()]: 32
  })
  await rarity.attributes.setVariable('character_created', {
    [summoner.toNumber()]: true
  })
  await rarity.skills.setVariable('skills', {
    [summoner.toNumber()]: skillsArray({ index: skillsenum.craft, points: 8 })
  })
  await rarity.mats.setVariable('balanceOf', {
    [summoner.toNumber()]: 1_000_000
  })
  return summoner
}

export async function mockCommonItem (
  base_type: number,
  item_type: number,
  crafter: number,
  rarity: IMockRarityContracts,
  ...
  await rarity.crafting.setVariable('items', {
    [item.toNumber()]: { base_type, item_type, crafted: 0, crafter }
  })
  await rarity.crafting.setVariable('_owners', {
    [item.toNumber()]: signer.address
  })
  await rarity.crafting.setVariable('_balances', {
...
export async function mockMasterworkProject (
  crafter: number,
  baseType: number,
  itemType: number,
  tools: number,
  toolsContract: string,
  masterwork: IMockMasterworkContracts
) {
  const project = await masterwork.projects.nextToken()
  const balance = await masterwork.projects.balanceOf(crafter)
  await masterwork.projects.setVariable('projects', {
    [project.toNumber()]: {
      baseType,
      itemType,
      tools,
  ...
  await masterwork.projects.setVariable('_owners', {
    [project.toNumber()]: crafter
  })
  await masterwork.projects.setVariable('_balances', {
    [crafter]: balance.add(1)
  })
  await masterwork.projects.setVariable('nextToken', project.add(1))
  return project
}

export async function mockMasterworkItem (
  crafter: number,
  ...
  const item = await masterwork.items.nextToken()
  const balance = await masterwork.items.balanceOf(signerAddress)
  await masterwork.items.setVariable('items', {
    [item.toNumber()]: {
      baseType,
      itemType,
      crafted: 0,
  ...
  await masterwork.items.setVariable('_owners', {
    [item.toNumber()]: signerAddress
  })
  await masterwork.items.setVariable('_balances', {
    [signerAddress]: balance.add(1)
  })
  await masterwork.items.setVariable('nextToken', item.add(1))
  return item
}

export async function mockCommonTools (
  crafter: number,
  masterwork: IMockMasterworkContracts,
  signer: SignerWithAddress
) {
  ...
  await masterwork.commonTools.setVariable('items', {
    [tools.toNumber()]: {
      base_type: 4,
      item_type: 2,
      crafted: 0,
  ...
  await masterwork.commonTools.setVariable('_owners', {
    [tools.toNumber()]: signer.address
  })
  await masterwork.commonTools.setVariable('_balances', {
    [signer.address]: balance.add(1)
  })
...
export async function mockMasterworkTools (
  crafter: number,
  masterwork: IMockMasterworkContracts,
  signer: SignerWithAddress
) {
...
export async function useRandomMock (
  ...
  await contract.setVariable(
    contractVariable,
    '0x7426dBE5207C2b5DaC57d8e55F0959fcD99661D4'
  )
}
