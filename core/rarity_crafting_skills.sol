...
contract rarity_crafting_skills is ForSummoners {
  ...
  mapping(uint => uint8[SKILL_SLOTS]) public crafting_skills;

  function get_skills(uint summoner) public view returns (uint8[SKILL_SLOTS] memory) {
    return crafting_skills[summoner];
  }

  function is_spell_caster(uint summoner) public view returns (bool) {
  ...
  function set_skills(
    uint summoner, 
    uint8[SKILL_SLOTS] memory skills
  ) public approvedForSummoner(summoner) 
  {
    uint8 base_craft_ranks = Skills.craft(summoner);
    uint8[SKILL_SLOTS] memory current_skills = crafting_skills[summoner];
    uint redeem_craft_ranks = 0;
    for (uint i = 0; i < SKILL_SLOTS; i++) {
      if(i == 0) require(is_spell_caster(summoner), "!is_spell_caster(summoner)");
      require(skills[i] >= current_skills[i], "skills[i] < current_skills[i]");
      redeem_craft_ranks += skills[i];
      require(redeem_craft_ranks <= base_craft_ranks, "redeem_craft_ranks > base_craft_ranks");
    }
